...
- Availability: if $T_{teacherAvailability}$ is true, then the teacher is available, but if false, it cant be schedulled.
...
- Fields:
  ...
  - `availability` → a boolean if that teacher is available (0: False, 1: True)
